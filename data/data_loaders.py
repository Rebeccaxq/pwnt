...
class ImageProcessDataset(torch.utils.data.dataset.Dataset):    
    ...
    def __getitem__(self, idx):
        name, seq_blur, seq_clear,seq_info= self.get_datum(idx)
        seq_blur, seq_clear = self.transforms(seq_blur, seq_clear)
        seq_blur_tensors=torch.stack(seq_blur)
        seq_clear_tensors=torch.stack(seq_clear)
        # sample={"name":name,"blur":seq_blur,"clear":seq_clear}
        sample={"name":name,"blur": seq_blur_tensors,"clear": seq_clear_tensors,"info": seq_info}
        return  sample
    ...
    def get_datum(self, idx):
        ...
        seq_clear = []
        seq_info = []
        seq_blur_paths=sorted(seq_blur_paths)
        seq_clear_paths=sorted(seq_clear_paths)
        # print(length)
        for i in range(length):
            ...
            seq_info.append(seq_blur_paths[i])
        ...
        return name, seq_blur, seq_clear,seq_info
